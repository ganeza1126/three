...
(function (global, factory) {
	...
	var KeyInputControl = function (_EventDispatcher) {
		...
		_createClass$5(KeyInputControl, [{
			...
			value: function updateAngle() {
				...
				if (up && !down) this.frontAngle = DEG_0;else if (!up && down) this.frontAngle = DEG_180;
			
				if (left){
					this.setLatLon(this._pointerLast.y, this._pointerLast.x + 0);
				}
			}
		}]);
	...
	function onkeyup(event) {
		...
		//if (!this.isUp && !this.isDown && (event.keyCode === KEY_W || event.keyCode === KEY_UP || event.keyCode === KEY_S || event.keyCode === KEY_DOWN || event.keyCode === KEY_A || event.keyCode === KEY_LEFT || event.keyCode === KEY_D || event.keyCode === KEY_RIGHT)) {
	...
	function onmousedrag(event) {
		...
		var x = -(this._pointerStart.x - event.clientX) / w * 2;// changed to - 
